  ...
  <DashboardLayout>
    <div class="space-y-4">
      <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <MetricCard v-if="temperatureData" :title="'Temperature'" :value="temperatureData.value + '°C'" :description="temperatureData.description">
          <template #icon>
            <Thermometer class="size-4 text-gray-500" />
          </template>
        </MetricCard>

        <MetricCard v-if="humidityData" :title="'Humidity'" :value="humidityData.value + '%'" :description="humidityData.description">
          <template #icon>
            <Droplets class="size-4 text-gray-500" />
          </template>
        </MetricCard>

        <MetricCard v-if="accessAttemptsData" :title="'Access Attempts'" :value="accessAttemptsData.value" :description="accessAttemptsData.description">
          <template #icon>
            <LockClosedIcon class="size-4 text-gray-500" />
          </template>
        </MetricCard>

        <MetricCard v-if="co2Data" :title="'CO₂'" :value="co2Data.value + ' ppm'" :description="co2Data.description">
          <template #icon>
